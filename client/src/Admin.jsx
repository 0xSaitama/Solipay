...
function Admin({ account, setMsg }) {
  ...
  const approveUni = async () => {
    ...
    const approved = await contract.methods
      .approveERC20Uni(addr1, amountDcm)
  ...
  const balanceView = async () => {
    ...
    const contract = await getContract(Stacking);
    const web3 = await getWeb3();
    const balanceCall = await contract.methods
      .getBalance(addr1)
      .call({ from: account });
  ...
  const swap = async () => {
    ...
    let amountMin = await contract.methods
      .getAmountMin(amountDcm, tolerance)
      .call();
    ...
    let swap = await contract.methods
      .swapTokens(addr1, addr2, amountDcm, amountMin, deadline)
      .send({ from: account }, async function (err, tx) {
        if (tx) {
          console.log(tx);
          await web3.eth.getTransactionReceipt(
